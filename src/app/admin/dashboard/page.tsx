    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-emerald-900/30 to-gray-900 rounded-2xl p-5 border border-emerald-800/30">
          ...
          <p className="text-3xl font-bold text-white mt-2">{formatCurrency(grossRevenue)}</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((card) => (
          <div key={card.label} className={`${card.bg} rounded-xl p-4 border border-gray-800/50 transition-all hover:scale-105 duration-200`}>
            <div className="flex items-center justify-between mb-2">
              <card.icon className={`w-5 h-5 ${card.color}`} />
              <div className={`flex items-center gap-1 text-xs ${card.trendUp ? 'text-green-400' : 'text-yellow-400'}`}>
